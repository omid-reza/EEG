    ...
    for pre_post_index=1:2
        ...
        for trial_index=1:EEG.(active_or_sham).trialsCount
            ...
            for cent_frequency=start_frequency:frequency_step:end_frequency
                ...
                groups.first.data =squeeze(mean(EEG.(active_or_sham).(ec_or_eo).data(pre_post_index, trial_index, groups.first.channels.indexs , :), 3))';
                groups.second.data=squeeze(mean(EEG.(active_or_sham).(ec_or_eo).data(pre_post_index, trial_index, groups.second.channels.indexs, :), 3))';
