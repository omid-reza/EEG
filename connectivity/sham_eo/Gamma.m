...
for pre_post_index=1:2
    ...
    for trial_index=1:EEG.sham.trialsCount
        ...
        for cent_frequency=30:0.5:200
            ...
            groups.first.data =squeeze(mean(EEG.sham.eo.data(pre_post_index, trial_index, groups.first.channels.indexs , :), 3))';
            groups.second.data=squeeze(mean(EEG.sham.eo.data(pre_post_index, trial_index, groups.second.channels.indexs, :), 3))';
    ...
    fileID=fopen(strcat("result\raw\connectivity\sham_eo\", pre_post_folder_names(pre_post_index), "Gamma.txt"), "w");
    fprintf(fileID, '%5d \n', result);
    fclose(fileID);
end