...
for pre_post_index=1:2
    for group_name_indexs=1:length(group_names)
        ...
        for trial_index=1:EEG.active.trialsCount
            ...
            range_begin=dsearchn(welch.cyclical_frequencies, 1);
            range_end=dsearchn(welch.cyclical_frequencies, 3);
            result(end+1)=sum(welch.PSD_estimate(range_begin:range_end));
        end
        % Write response(Alpha band powers) into a file
        fileID=fopen(strcat("result\raw\power\active_ec\",pre_post_folder_names(pre_post_index), group_file_names(group_name_indexs), "-Delta.txt"), "w");
        fprintf(fileID, '%5d \n', result);
        fclose(fileID);
    end
end