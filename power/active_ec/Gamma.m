...
for pre_post_index=1:2
    for group_name_indexs=1:length(group_names)
        ...
        for trial_index=1:EEG.active.trialsCount
            ...
            welch.signal.data=squeeze(mean(EEG.active.ec.data(pre_post_index, trial_index, groups.(group_names(group_name_indexs)).channels.indexs , :), 3))';
            ...
            range_begin=dsearchn(welch.cyclical_frequencies, 30);
            range_end=dsearchn(welch.cyclical_frequencies, 200);
            result(end+1)=sum(welch.PSD_estimate(range_begin:range_end));
        end
        % Write response(Alpha band powers) into a file
        fileID=fopen(strcat("result\raw\power\active_ec\",pre_post_folder_names(pre_post_index), group_file_names(group_name_indexs), "-Gamma.txt"), "w");
        fprintf(fileID, '%5d \n', result);
        fclose(fileID);
    end
end