function EEG = load_data_sham_eo(EEG)
    ...
    %   This Function loads EEG.sham.eo.data with files that are stored in \datasest\sham\{trial_numver}\{pre_or_post}\eo.txt and return EEG.
    for trials_index=1:EEG.sham.trialsCount
        for pre_post_index=1:2
            if ismember(trials_index, EEG.sham.eo.excluded_trials)==0
                file_name=strcat('dataset\sham\', string(trials_index), pre_or_post_folder_name_getter(pre_post_index), 'eo.txt');
                eo=load(file_name);
