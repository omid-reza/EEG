function EEG = load_data_active_eo(EEG)
    ...
    %   This Function loads EEG.active.eo.data with files that are stored in \data\active\{trial_numver}\{pre_or_post}\eo.txt and return EEG.
    for trials_index=1:EEG.active.trialsCount
        for pre_post_index=1:2
            if ismember(trials_index, EEG.active.eo.excluded_trials)==0
                file_name=strcat('data\active\', string(trials_index), pre_or_post_folder_name_getter(pre_post_index), 'eo.txt');
                eo=load(file_name);
