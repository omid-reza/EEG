function EEG = load_data_active_ec(EEG)
    ...
    %   This Function loads EEG.active.ec.data with files that are stored in \data\active\{trial_numver}\{pre_or_post}\ec.txt and return EEG.
    for trials_index=1:EEG.active.trialsCount
        for pre_post_index=1:2
            if ismember(trials_index, EEG.active.ec.excluded_trials)==0
                file_name=strcat('data\active\', string(trials_index), pre_or_post_folder_name_getter(pre_post_index), 'ec.txt');
                ec=load(file_name);
